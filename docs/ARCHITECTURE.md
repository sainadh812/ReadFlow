...
Normalizer `english-3` recognizes a narrow word + sentence punctuation + attached numeral pattern, as in `money.4`, without treating decimal numbers as references. It pronounces the numeral literally and preserves both spoken words' ownership by the same source word. Vertical bars have the literal pronunciation "vertical bar", never an inferred OCR correction to "I" or a guessed mathematical interpretation. Unsupported expressions still fail. The revision participates in cache keys; source IDs, stored page extraction and reading positions do not change.
...
OCR uses bundled ML Kit 16.0.1. Render scale starts at 300 DPI and caps at ten million pixels. A conservative automatic deskew trial and up to six confidence/geometry-gated line retries improve difficult inputs; retry bitmaps are limited to two million pixels. Extraction resources are serialized; preview rendering has a separate lock and independently opened PDF resources so display need not wait for OCR. ML Kit tasks drain before bitmaps are recycled even on cancellation. Only the requested page and pages needed to fill the bounded audio queue are processed. No whole-book scan occurs before playback. Scaling, arbitrary deskew, focused crops and quarter-turn rotation retain reversible processed-to-page mappings. Original source image dimensions are preserved. User cropping is not implemented. See [accented speech and OCR refinement](ACCENTS_AND_OCR.md) for validation limits.
...
Reports use bounded JSON, optional exact WAV attachments, same-directory atomic installation, count/byte retention and warning deduplication. Export snapshots are taken under the store lock; external SAF output runs after releasing it. Export verifies the attached audio hash. Logs are private until an explicit, privacy-confirmed export or GitHub send. User-facing storage failure does not replace the original issue. See [ISSUE_LOGS.md](ISSUE_LOGS.md) for limits, deletion semantics and coverage.

From 0.1.5, `NormalizationException` retains the exact rejected source token, its source IDs and attempted expansion, including through `SpeechPreparationException`. Reports include those in an optional `normalization` object; older schema-1 reports remain readable. The coordinator resets sample rate/count before page extraction and before advancing the lazy sentence iterator, so normalization failures cannot inherit a previous chunk's audio metadata.

From 0.1.6, the same atomic per-event store also exposes a single chronological `issues.jsonl` export. Internal per-event records remain for safe recovery and individual audio exports; retention is still bounded, not infinite. `GitHubDiagnostics` serializes uploads, groups repeats, recovers one installation-scoped issue, and discovers previously sent IDs in checksum-marked comments before appending only pending events. API calls are behind `GitHubIssueApi` for isolated tests. Responses and pagination are bounded; comment pages are processed without retaining the complete remote history in memory. The app does not commit logs into the source tree.

`GitHubSettings` keeps the repository/issue link in private atomic JSON and encrypts the user-entered token with an Android Keystore AES-GCM key. Credentials are neither included in reports nor built into the APK. No redirect or automatic POST retry is allowed; a lost response is recovered by reading the server history on the next explicit send. Public projection omits input and error messages; the private projection is independently bounded and gated by a live repository-visibility check before each batch. Neither path uploads audio. See [GITHUB_DIAGNOSTICS.md](GITHUB_DIAGNOSTICS.md) for setup and remaining device checks.
