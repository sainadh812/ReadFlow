...
The latest patch is **0.1.5-prototype**, fixing reported OCR normalization stops on attached numerals (`money.4`) and vertical bars (`|`), and adding exact rejected-token diagnostics. See [OCR_TEXT_FIX.md](OCR_TEXT_FIX.md) for build results and a separate Pocket host alignment failure found during testing. The [issue logs](ISSUE_LOGS.md), [text/PDF fixes](TEXT_PDF_FIX.md), [native callback fix](PLAYBACK_CRASH_FIX.md) and [installation fix](INSTALLATION_FIX.md) remain included. The user reports that basic Kokoro speech worked on their SM-S938B, Android 16/API 36, with 0.1.2 and supplied 0.1.4 normalization error reports; that is user-reported evidence, not independently measured timing or both-engine acceptance. Earlier build results below describe the original 0.1.0 delivery; `evidence/build-results.json` records the latest build.
